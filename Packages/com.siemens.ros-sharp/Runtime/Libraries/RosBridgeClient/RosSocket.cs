                ...
                case "publish":
                    {
                        ...
                        var subs = SubscribersOf(topic);
                        UnityEngine.Debug.Log($"[RosSocket] ← publish topic={topic}, subscribers={subs.Count}, total_subscribers={Subscribers.Count}");
                        foreach (Subscriber subscriber in subs)
                        {
                            try
                            {
                            ...
                            {
                                UnityEngine.Debug.LogError($"[RosSocket] subscriber.Receive 异常: topic={topic}, type={subscriber.TopicType.Name} — {ex.Message}\n{ex.StackTrace}");
                            }
                        }
                        return;
                    }
